# 🔫 Ações N-Z

### 1. Motel <a href="#id-1-motel" id="id-1-motel"></a>

* Bandidos: Mín. de 2 | Máx. 4
* Policiais: 1 a mais que o número de bandidos.
* Armamento: Apenas Pistola.
* Negociação: Não há.
* Refém: Não Permitido.

Observações:

* Obrigatoriamente TetiChão
* Fugas são proibidas.
* Não é permitido marcar drop da polícia.

<figure><img src="../.gitbook/assets/motel.png" alt=""><figcaption></figcaption></figure>

### 2. Mosley <a href="#id-2-mosley" id="id-2-mosley"></a>

* Bandidos: Mín. de 2 | Máx. 4
* Policiais: 1 a mais que o número de bandidos.
* Armamento: Apenas Pistola.
* Negociação: Não há.
* Refém: Não Permitido.

Observações:

* Obrigatoriamente TetiChão
* Fugas são proibidas.
* Não é permitido marcar drop da polícia.

<figure><img src="../.gitbook/assets/mosley.png" alt=""><figcaption></figcaption></figure>

### 3. Navio <a href="#id-3-navio" id="id-3-navio"></a>

* Bandidos: Mín. de 6 | Máx. 8
* Policiais: 2 a mais que o número de bandidos.
* Armamento: Submetralhadora
* Negociação: Não há.
* Refém: Não Permitido.

Observações:

* Obrigatoriamente TetiChão

<figure><img src="../.gitbook/assets/navio.png" alt=""><figcaption></figcaption></figure>

### 4. Nióbio <a href="#id-4-niobio" id="id-4-niobio"></a>

* Bandidos: Mín. de 12 | Máx. 15
* Policiais: 5 a mais que o número de bandidos.
* Armamento: Somente Fuzil.
* Smoke: No máximo 4.
* Refém: Proibido.
* Negociação: Inexistente.

Observações:

* É proibido posicionar-se no elevador.
* É proibido marcar a escada da p2, apenas a porta automática pode ser usada como posição de defesa.
* A entrada aquática só pode ser utilizada para acesso ou retirada pelo túnel.
* É estritamente proibido que todos os ladrões se posicionem no último pixel do niobio afim de abusar da posição para matar utilizando apenas do mecanismo todos os policias, assim, quebrando a ideia do RP de roubo.

<figure><img src="../.gitbook/assets/niobio.png" alt=""><figcaption></figcaption></figure>

### 5. Planet <a href="#id-5-planet" id="id-5-planet"></a>

* Bandidos: Mín. de 5 | Máx. 7
* Policiais: 2 a mais que o número de bandidos.
* Armamento: Somente Pistolas;
* Negociação: Não há.
* Refém: Não permitido.

Observações:

* Obrigatoriamente TetiChão.
* Fugas proibidas.
* Não é permitido marcar drop da polícia.

<figure><img src="../.gitbook/assets/planet.png" alt=""><figcaption></figcaption></figure>

### 6. Skate <a href="#id-6-skate" id="id-6-skate"></a>

* Bandidos: Mín. de 2 | Máx. 4
* Policiais: 1 a mais que o número de bandidos.
* Armamento: Somente Pistolas;
* Negociação: Não há.
* Refém: Não permitido.

Observações:

* Obrigatoriamente TetiChão.
* Fugas proibidas.
* Não é permitido marcar drop da polícia.

<img src="http://localhost:5173/Skate/skate.png" alt="" width="800">

<br>
