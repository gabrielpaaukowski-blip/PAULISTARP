...
* Redução de Farm: A quantidade de recursos obtidos pela facção será temporariamente reduzida.
* Bloqueio de Farm: A facção ficará impedida de farmar recursos durante o período de punição.
* Proibição de Ações: A facção será temporariamente impedida de realizar ações específicas, como assaltos, sequestros, guerras, entre outras.
* Bloqueio de Baú: O baú da facção será bloqueado, impossibilitando o uso e retirada de itens.
* Perda Parcial do Baú: Parte dos itens armazenados no baú da facção será removida como forma de penalidade.
* Multa Financeira no RP: A facção será obrigada a pagar uma multa em dinheiro dentro do roleplay.
...
* Invasão Imediata: É a invasão que pode ocorrer caso um player em situação de fuga evada para dentro do seu QG, Facção ou Gueto. A facção que esteja invadindo deve ter um contingente mínimo de 6 pessoas ou a invasão pode ser analisada e os players responderem por anti-amor a vida.
* Invasão Não Marcada: Caso o player que está passando pelo local seja alvejado de dentro para fora do QG, Facção ou Gueto. A facção que esteja invadindo deve ter um contingente mínimo de 6 pessoas ou a invasão pode ser analisada e os players responderem por anti-amor a vida.
* Invasão de Recuperação de Veículo: Liberada se o veículo estiver rastreado dentro de território dominado, após tentativa de diálogo frustrada com os residentes locais. A facção que esteja invadindo deve ter um contingente mínimo de 6 pessoas ou a invasão pode ser analisada e os players responderem por anti-amor a vida.

📌 Invasão Imediata

1. Liberada invasão imediata ao território.
2. Mínimo de 6 invasores.
3. É obrigatório o uso do uniforme ou roupas que identifiquem a organização que está iniciando a invasão.
4. Apenas os presentes na invasão podem circular na base inimiga para recuperar corpos, ou seja, os corpos só podem ser recuperados em caso de vitória, caso percam, os invasores devem ir obrigatoriamente de GG.
5. É proibido solicitar QRR após o início da invasão.
6. Caso a ação seja perdida, a tentativa de captura é encerrada, sendo totalmente proibido a continuidade do RP.

📌Invasão Não Marcada:

1. Liberada invasão imediata sem necessidade de diálogo.
2. Mínimo de 6 membros invasores é exigido.
3. É obrigatório o uso do uniforme ou roupas que identifiquem a organização que está iniciando a invasão.
4. Apenas os presentes na invasão podem circular na base inimiga para recuperar corpos, ou seja, os corpos só podem ser recuperados em caso de vitória, caso percam, os invasores devem ir obrigatoriamente de GG.
5. É proibido solicitar QRR após o início da invasão.
...
1. A tentativa de dialogo com os residentes é obrigatória antes da invasão;
2. Mínimo de 6 membros invasores é exigido.
3. É obrigatório o uso do uniforme ou roupas que identifiquem a organização que está iniciando a invasão.
4. Apenas os presentes na invasão podem circular na base inimiga para recuperar corpos, ou seja, os corpos só podem ser recuperados em caso de vitória, caso percam, os invasores devem ir obrigatoriamente de GG.
5. É proibido solicitar QRR após o início da invasão.
6. Em caso de morte ou retirada do local, finaliza-se a ação, é proibido retornar.
7. Caso percam, o veículo deve ser esquecido até o próximo RR;
8. É proibido levar veículos da base invadida para a base invasora como retaliação pelo roubo.
9. Em caso de perca da ação, quem voltar para dar continuidade no RP, poderá ser punido mediante análise dos fatos pela equipe de denúncia.
